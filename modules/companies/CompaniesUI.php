...
class CompaniesUI extends UserInterface {
        ...
        if (!empty($jobOrdersRS)) {
            foreach ($jobOrdersRS as $rowIndex => $row) {
                ...
                $jobOrdersRS[$rowIndex]['recruiterAbbrName'] = StringUtility::makeInitialName(
                                $jobOrdersRS[$rowIndex]['recruiterFirstName'], $jobOrdersRS[$rowIndex]['recruiterLastName'], false, LAST_NAME_MAXLEN
                );

                $jobOrdersRS[$rowIndex]['ownerAbbrName'] = StringUtility::makeInitialName(
                                $jobOrdersRS[$rowIndex]['ownerFirstName'], $jobOrdersRS[$rowIndex]['ownerLastName'], false, LAST_NAME_MAXLEN
                );
            }
        }
            ...
            foreach ($contactsRS as $rowIndex => $row) {
                ...
                if (!empty($contactsRS[$rowIndex]['ownerFirstName'])) {
                    $contactsRS[$rowIndex]['ownerAbbrName'] = StringUtility::makeInitialName(
                                    $contactsRS[$rowIndex]['ownerFirstName'], $contactsRS[$rowIndex]['ownerLastName'], false, LAST_NAME_MAXLEN
                    );
                } else {
    ...
    private function onAdd() {
         ...
         */
        $target_file_name = basename($_FILES["companyLogo"]["name"]);
        $target_file = COMPANY_LOGO_TARGET_LOCATION . $target_file_name;
        move_uploaded_file($_FILES["companyLogo"]["tmp_name"], $target_file);
        
        $companies = new Companies($this->_siteID);
        $companyID = $companies->add(
                $name, $address, $city, $state, $zip, $phone1, $phone2, $faxNumber, 
                $url, $keyTechnologies, $isHot, $notes, 
                $this->_userID, $this->_userID, $target_file_name
        );
    ...
    private function onEdit() {
        ...
        if (!$companies->update($companyID, $name, $address, $city, $state, $zip, 
                $phone1, $phone2, $faxNumber, $url, $keyTechnologies, $isHot, $notes, 
                $owner, $billingContact, $email, $emailAddress, $target_file_name)) {
        ...
        if (isset($_POST['updateContacts'])) {
            if ($_POST['updateContacts'] == 'yes') {
                $contacts = new Contacts($this->_siteID);
                $contacts->updateByCompany($companyID, $address, $city, $state, $zip, $target_file_name);
            }
        }
    ...
    private function onSearch() {
        ...
        $savedSearches = new SavedSearches($this->_siteID);
        $savedSearches->add(
                DATA_ITEM_COMPANY, $query, $_SERVER['REQUEST_URI'], false
        );
        $savedSearchRS = $savedSearches->get(DATA_ITEM_COMPANY);
